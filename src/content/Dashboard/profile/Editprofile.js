...
import PassionsDialog, { ChipFactory } from "../CreationStep/PassionsDialog";
import { FixedAppbar } from "../profile/ProfileArea";
const EditMobile = () => {
  return (
    <Box sx={classes.root}>
...
const EditInfos = () => {
  const formik = useFormik({
    initialValues: {
      ...
      SexualPreferences: "",
      Biografy: "",
      pic1: "https://avatars.githubusercontent.com/u/58108317?v=4",
      pic2: null,
      pic3: null,
      pic4: null,
      pic5: null,
...
const UploadImages = () => {
  ...
  const handlUpload = (event) => {
    ...
    event.target.value = ""; // for onchage so i can upload same file if i deleted it
  };
  const DeleteImg = (name) => {
    console.log(name);
    if (formik?.values?.profilePic === name)
  ...
  return (
    <Box sx={classes.UploadRoot}>
      ...
      <Grid container spacing={3} justifyContent="center" alignItems="center">
        {["pic1", "pic2", "pic3", "pic4", "pic5", "pic6"].map((elem, index) => (
          <Grid item key={index} sx={{ position: "relative" }}>
            <input
              ...
              disabled={formik.values[elem]}
            />
            {formik.values[elem] ? (
              <ClearRoundedIcon
                sx={classes.DeleteButton}
                onClick={() => DeleteImg(elem)}
              />
            ) : (
              <label htmlFor={elem}>
                <AddRoundedIcon sx={classes.addButton} />
              </label>
            )}
            <label htmlFor={elem}>
              <Box
  ...
  return (
    <Box sx={{ flexGrow: 1 }}>
      <Stack direction="column" spacing={3}>
        ...
        >
          {formik.values.hoobies.length > 0 ? "Edit " : "Add "} Passions
        </Button>
      </Stack>
